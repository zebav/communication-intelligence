...
export function CalendarCommitmentReview({name,account,events,master,now,busy,onCopy,onCopyMany,format}:{
 name:string;account:string;events:CalendarEvent[];master:CalendarEvent[];now:number;busy:boolean;
 onCopy:(event:CalendarEvent)=>void;onCopyMany?:(events:CalendarEvent[])=>void;format:(value:string)=>string;
}) {
 const [filter,setFilter]=useState("attention"),[query,setQuery]=useState(""),[limit,setLimit]=useState(25);
 const [selected,setSelected]=useState<string[]>([]),[reviewing,setReviewing]=useState(false);
 ...
 const eligible=visible.filter(r=>r.status==="missing"&&Date.parse(r.event.start)>now);
 const chosen=reviews.filter(r=>r.status==="missing"&&Date.parse(r.event.start)>now&&selected.includes(r.event.id)).map(r=>r.event);
 return <details className="calendar-source"><summary>{name} · {account} · {attention.length} att ta ställning till</summary>
  ...
  {onCopyMany&&<section aria-label="Samlad överföring"><button className="btn" disabled={busy||!eligible.length} onClick={()=>{setSelected(eligible.map(r=>r.event.id));setReviewing(false);}}>Välj alla {eligible.length} saknade framtida poster i urvalet</button><button className="btn" disabled={busy||!chosen.length} onClick={()=>setReviewing(true)}>Granska {chosen.length} valda kopior</button><button className="btn" disabled={busy||!selected.length} onClick={()=>{setSelected([]);setReviewing(false);}}>Rensa urval</button>
   {reviewing&&<div className="calendar-approval"><h3>Godkänn {chosen.length} kopior från {account}</h3><p>Originalen ändras inte. Inga inbjudningar skickas. Varje bokning kontrolleras på nytt; vid fel stoppas resten. Avstämningen godkänns inte automatiskt. Låt sidan vara öppen tills resultatet visas.</p><ul>{chosen.map(e=><li key={e.id}>{e.title} · {format(e.start)} – {format(e.end)}</li>)}</ul><button className="btn primary" disabled={busy||!chosen.length} onClick={()=>{setReviewing(false);onCopyMany(chosen);}}>Godkänn och för över de valda kopiorna utan utskick</button></div>}
  </section>}
  <ul className="calendar-events">{visible.slice(0,limit).map(r=><li key={r.event.id}><strong>{r.event.title||"Utan rubrik"}</strong><span>{format(r.event.start)} – {format(r.event.end)}</span>
   {onCopyMany&&r.status==="missing"&&Date.parse(r.event.start)>now&&<label><input type="checkbox" disabled={busy} checked={selected.includes(r.event.id)} onChange={e=>{setSelected(e.target.checked?[...selected,r.event.id]:selected.filter(id=>id!==r.event.id));setReviewing(false);}}/> Välj {r.event.title} för överföring</label>}
