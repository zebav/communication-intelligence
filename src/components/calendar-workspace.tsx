...
export function CalendarWorkspace({conversations}:{conversations:CalendarConversation[]}) {
 ...
 const run=async(body:Record<string,unknown>)=>{
  ...
  try {
   ...
   else {setSlots([]);setStatus(result.warning||"Åtgärden är sparad.");await load();setApproval(null);}
 ...
 return <section className="calendar-page">
  ...
  {data&&<>
   <details className="calendar-panel"><summary>Nytt fysiskt möte · plats, resa och inbjudningar</summary><p>Välj kontakt och plats här när det är relevant. Du granskar alltid tid, resa och inbjudningar innan något bokas.</p><CalendarMeetingComposer timezone={timezone} onDone={load}/></details>
   <details className="calendar-panel"><summary>Förslag från konversationer</summary><p>Systemet hittar möjliga möten, men bokar aldrig från ett meddelande utan att du granskar förslaget.</p><CalendarIntents conversations={conversations} onUse={(id,intent)=>{setConversationId(id);setTitle(intent.summary.slice(0,300));if(intent.date)setDate(intent.date);const defaults=meetingDefaults[intent.meetingType];setDuration(intent.durationMinutes??defaults.durationMinutes);setPreparation(defaults.preparationMinutes);setRecovery(defaults.recoveryMinutes);setPhysical(Boolean(intent.locationText));setSlots([]);setStatus("Granska datum, plats, längd och buffertar innan du söker tider. Ingen bokning har gjorts.");}}/></details>
   ...
   <details className="calendar-panel"><summary>Planeringsregler och buffertar</summary><CalendarPlanning timezone={timezone} onSaved={rules=>{setSlots([]);setPreparation(rules.preparationMinutes);setRecovery(rules.recoveryMinutes);}}/></details>
  ...
  {approval&&<section className="calendar-panel calendar-approval" aria-label="Godkänn bokning"><h2>Godkänn bokning</h2><strong>{approval.title}</strong><p>{fmt(approval.starts_at)} – {fmt(approval.ends_at)}</p><p>{approval.meeting_details?.locationLabel}</p><p style={{whiteSpace:"pre-wrap"}}>{approval.meeting_details?.description}</p><p>Skapas i din masterkalender. Tid och eventuell resa kontrolleras igen.</p>{Boolean(approval.meeting_details?.attendees.length)?<><ul>{approval.meeting_details!.attendees.map(a=><li key={a}>{a}</li>)}</ul><label><input type="checkbox" checked={inviteApproval} onChange={e=>setInviteApproval(e.target.checked)}/> Godkänn kalenderinbjudningar till dessa adresser</label></>:<p>Inga inbjudningar skickas.</p>}<div className="calendar-actions"><button className="btn primary" disabled={busy||(Boolean(approval.meeting_details?.attendees.length)&&!inviteApproval)} onClick={()=>run({action:"confirm",holdId:approval.id,approved:true,approvedInvitations:inviteApproval})}>{busy?"Kontrollerar…":"Godkänn och boka / kontrollera"}</button><button className="btn" disabled={busy} onClick={()=>{setApproval(null);setInviteApproval(false);}}>Avbryt</button></div></section>}
 </section>;
}
