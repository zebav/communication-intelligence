...
type Entry = { id: string; category: string; key: string; value: string; sensitivity: Sensitivity; allowedUses: string[]; hasValue?: boolean; masked?: boolean; metadata?: Record<string, unknown>; };
...
const groups = [
  ...
  { id: "credentials", label: "Webbplatser & inloggningar", fields: ["Webbplats", "Användarnamn", "Lösenord"] },
  { id: "web_forms", label: "Formuläruppgifter", fields: ["Kundnummer", "Kontonummer", "Medlemsnummer", "Faktureringsuppgift"] },
];
...
export function PersonalKnowledgeVault() {
  ...
  const open = (entry?: Entry) => { setMessage(""); setError(""); setDraft(entry ? { id: entry.id, category: entry.category, key: entry.key, value: entry.masked ? "" : entry.value, sensitivity: entry.sensitivity, allowedUses: entry.allowedUses.join(", ") } : emptyDraft()); dialog.current?.showModal(); };
  const save = async () => {
  ...
  return <section className="personal-context" aria-label="Personal Context">
    <div className="personal-context-head"><div><span className="eyebrow">Personal Context</span><h2>Dina privata uppgifter</h2><p>Spara information som kan behövas för planering och godkända uppgifter, inklusive webbplatsinloggningar. Känsliga värden krypteras och lösenord visas aldrig tillbaka i klartext.</p></div><button className="btn primary" onClick={() => open()}><Plus size={14} /> Lägg till uppgift</button></div>
