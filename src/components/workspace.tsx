...
import { whatsappComposeUrl } from "@/lib/connectors/whatsapp-compose";
...
import { ContactDuplicates } from "@/components/contact-duplicates";
...
type View = "today" | "cases" | "inbox" | "people" | "followups" | "outcomes" | "cleanup" | "intelligence" | "connections" | "settings" | "calendar" | "assistant";
...
function LiveEmailCard({ email, onClick }: { email: SyncedEmailConversation; onClick: () => void }) { const action = actionLabels[email.recommendedAction as RecommendedAction] ?? email.recommendedAction; return <article className="card email-card"><div className="card-top"><span>{email.title}</span><span className="score">{email.priorityScore}</span></div><div className="card-person"><div className="avatar">{email.personName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</div><div><PersonLink personId={email.personId} name={email.personName} /><span>{email.classification}</span></div></div><p>{emailDashboardExcerpt(email)}</p><button className="pill" onClick={onClick}>{action} <ChevronRight size={10} /></button></article> }
...
function InboxView({ syncedEmails, people, onOpenAssistant }: { syncedEmails: SyncedEmailConversation[]; people: CommunicationPersonOption[]; onOpenAssistant: () => void }) {
...
function SyncedInbox({ emails, people, onOpenAssistant }: { emails: SyncedEmailConversation[]; people: CommunicationPersonOption[]; onOpenAssistant: () => void }) {
  const router = useRouter();
  ...
  return <div className="inbox">
    <section className="inbox-col"><div className="inbox-head"><h1>Inbox</h1><select className="filter" aria-label="Email category" value={category} onChange={(event) => setCategory(event.target.value)}>{EMAIL_CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></div>{filtered.map((email) => <div key={email.id} className={`conversation-row ${selected.id === email.id ? "selected" : ""}`} onClick={() => selectEmail(email.id)}><div className="avatar">{email.personName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</div><div><PersonLink personId={email.personId} name={email.personName} />{email.unread ? " · New" : ""}<p>{email.title}</p></div><div className="row-meta">{new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(email.receivedAt))}<div className="row-score">{email.priorityScore}</div></div></div>)}</section>
    <section className="inbox-col thread"><div className="thread-head"><PersonLink personId={selected.personId} name={selected.personName} /><span>{selected.title} · Outlook · {selected.threadMessages.length} message{selected.threadMessages.length === 1 ? "" : "s"}</span></div><div className="messages">{selected.threadMessages.length ? selected.threadMessages.map((message) => <div className={`message ${message.direction === "out" ? "out" : ""}`} key={message.id}><ReadableMessage text={message.body} /><div className="message-meta">{message.direction === "out" ? "You" : selected.personName} · Outlook · {new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(message.sentAt))}</div></div>) : <div className="message"><ReadableMessage text={selected.preview || "No text preview is available for this message."} /></div>}</div></section>
    <aside className="intel"><h2>Communication Intelligence</h2>
      <div className="intel-section decision-inbox-card"><div className="intel-label">Decision &amp; Execution Center V2</div>
        <strong>{selected.analysis?.summary || selected.preview || selected.title}</strong>
        ...
        {selected.analysis?.actionSuggestion?.targetUrl && <div className="learning-context"><b>Webbplats:</b> {selected.analysis.actionSuggestion.targetUrl}</div>}
