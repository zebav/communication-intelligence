...
import { kinds, kindLabels, sendCapability, statusLabels, taskBucket, type Task, type Plan, type TaskKind } from "@/lib/assistant/model";
...
export type AssistantSnapshot = {
  ...
  next: string | null; scanned: number; tasksLimited: boolean;
...
export function AssistantWorkspace({ people }: { people: CommunicationPersonOption[] }) {
  ...
  return <div className="page assistant-workspace"><header><p className="eyebrow">Executive Assistant V1</p><h1>Handlingsinkorg</h1><p>Från meddelande till genomfört uppdrag. Du granskar mottagare, innehåll och bokning innan något skickas.</p></header>
...
export function AssistantBoard({ snapshot, people, selected, onSelect, act, busy, onMore, onRefresh }: {
  ...
  return <>
    ...
    <section className="assistant-proposals"><h2>Förslag från konversationer</h2><p>Senaste hämtningen granskade {snapshot.scanned} inkommande meddelanden. Reklam och redan besvarade meddelanden föreslås inte. Förslagen är inte godkännanden.</p>
