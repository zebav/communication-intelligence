...
type HealthAccount = { id: string; account_name?: string | null; account_identifier?: string | null; live_delivery?: "subscribed" | "not_subscribed" | "unknown"; live_delivery_error?: string | null; callback_url?: string | null; expected_callback_url?: string; callback_matches?: boolean | null; messages_field?: string; app_subscription_error?: string | null };
...
export function ChannelDiagnostics({ whatsappConnections, persistence }: { whatsappConnections: WhatsAppConnection[]; persistence: SocialPersistence[] }) {
  ...
  return <div className="page" style={{ maxWidth: 900 }}>
    ...
    <div className="card">
      ...
      {(health?.accounts ?? []).map((account) => <div className="learning-notice" key={account.id}><CheckCircle2 size={16} /><div><strong>{account.account_name || account.account_identifier || "WhatsApp account"}</strong><p>Live delivery: {account.live_delivery ?? "unknown"}{account.live_delivery_error ? ` · ${account.live_delivery_error}` : ""}</p><p>Messages field: {account.messages_field ?? "unknown"}{account.app_subscription_error ? ` · ${account.app_subscription_error}` : ""}</p><p>Callback URL: {account.callback_url ?? "Not verified"}</p><p>Expected callback: {account.expected_callback_url ?? "unknown"}</p><p>Callback matches: {account.callback_matches == null ? "unknown" : account.callback_matches ? "yes" : "no"}</p>{account.messages_field === "not_subscribed" && <p>Subscribe to the messages field for whatsapp_business_account in your Meta app Webhooks settings.</p>}<button className="btn primary" disabled={Boolean(working)} onClick={() => void subscribe(account.id)} style={{ marginTop: 9 }}><Wrench size={12} /> {working === account.id ? "Repairing…" : "Repair live delivery"}</button></div></div>)}
