...
export async function calendarTravelAssessment(db:SupabaseClient,owner:string,input:TravelPlanRequest,provider:RouteService) {
 ...
 const checked=await calendarSuggestions(db,owner,{date,duration:(Date.parse(input.end)-Date.parse(input.start))/60000,preparation:0,recovery:0,physical:false});
 ...
 const busy=[...(master?.snapshot??[] as CalendarEvent[]).filter((e:CalendarEvent)=>e.blocksAvailability&&e.status!=="cancelled"),
  ...holds.data.filter(h=>h.status==="executing"||h.status==="confirmed"||(h.status==="active"&&Date.parse(h.expires_at)>Date.now())).map(h=>({start:new Date(Date.parse(h.starts_at)-h.preparation_minutes*60000).toISOString(),end:new Date(Date.parse(h.ends_at)+h.recovery_minutes*60000).toISOString()}))];
