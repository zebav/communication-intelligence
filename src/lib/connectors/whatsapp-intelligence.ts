...
export async function analyzeIncomingWhatsAppMessage(input: { ownerId: string; conversationId: string; messageId: string }) {
  ...
  const [{ data: conversation }, { data: message }, { data: profile }] = await Promise.all([
    ...
    database.from("messages").select("id,body_text,metadata,direction").eq("id", input.messageId).eq("owner_id", input.ownerId).eq("source", "whatsapp").maybeSingle(),
  ...
  const [{ data: person }, { data: history }, { data: memories }, { data: styleRows }] = await Promise.all([
    ...
    database.from("messages").select("direction,body_text").eq("owner_id", input.ownerId).eq("conversation_id", conversation.id).eq("source", "whatsapp").order("sent_at", { ascending: false }).limit(100),
