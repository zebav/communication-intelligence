...
describe("OpenAIResponsesService", () => {
  it("requests non-retained structured analysis with minimized input", async () => {
    const output = { category: "Action Required", confidence: 0.9, summary: "Reply requested.", intent: "Request", priorityScore: 8, priorityReason: "A response is requested today.", recommendedAction: "RESPOND_TODAY", requiresReply: true, commitment: { detected: false, description: "", dueAt: "", owner: "unknown", confidence: 0 } };
    let requestBody = "";
    ...
    await expect(service.analyzeEmail({ ownerId: "owner", senderName: "A", subject: "Please reply", preview: "Can you review this?", currentClassification: "Business" })).resolves.toEqual(output);
    const body = JSON.parse(requestBody);
    expect(body.store).toBe(false); expect(body.model).toBe("test-model"); expect(body.text.format.type).toBe("json_schema");
    expect(body.input).not.toContain("owner"); expect(body.safety_identifier).toMatch(/^[a-f0-9]{64}$/);
  });
});
