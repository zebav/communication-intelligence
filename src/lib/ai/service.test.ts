...
describe("OpenAIResponsesService", () => {
  it("requests non-retained structured analysis with minimized input", async () => {
    const output = { category: "Action Required", confidence: 0.9, summary: "Reply requested.", intent: "Request", priorityScore: 8, priorityReason: "A response is requested today.", recommendedAction: "RESPOND_TODAY", requiresReply: true, draftResponse: "Yes, I will review it today.", draftTone: "direct and warm", commitment: { detected: false, description: "", dueAt: "", owner: "unknown", confidence: 0 }, memoryCandidates: [], relationshipSuggestion: { type: "customer", confidence: 0.8, reason: "The sender discusses customer work." }, forwardingSuggestion: { recommended: false, recipientRole: "none", reason: "", introduction: "" }, actionSuggestion: { detected: false, type: "none", task: "", reason: "", targetUrl: "", requiresLogin: false, contactIds: [], requiredFields: [], confidence: 0 } };
    let requestBody = "";
