  ...
  async deeplyAnalyzeEmail(input: DeepAnalysisRequest) {
    ...
    try {
      ...
      const body: Record<string, unknown> = {
        ...
        input: JSON.stringify({ analysis_date: new Date().toISOString(), sender_name: input.senderName.slice(0, 200), subject: input.subject.slice(0, 300), current_message: input.preview.slice(0, 7000), conversation_history: (input.conversationMessages ?? []).slice(-20).map((message) => ({ direction: message.direction, body: message.body.slice(0, 3500) })), relationship_context: input.relationshipContext?.slice(0, 500) ?? "unknown", verified_person_memories: (input.verifiedPersonMemories ?? []).slice(0, 16).map((value) => value.slice(0, 400)), verified_communication_profile: input.personaContext?.slice(0, 8000) ?? "not configured", owner_writing_examples: (input.styleExamples ?? []).slice(0, 8).map((value) => value.slice(0, 900)) }),
