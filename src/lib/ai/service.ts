...
export interface EmailAnalysisRequest { ownerId: string; senderName: string; subject: string; preview: string; currentClassification: string; relationshipContext?: string; personaContext?: string; styleExamples?: string[]; conversationMessages?: { direction: "in" | "out"; body: string }[] }
  ...
  async analyzeEmail(input: EmailAnalysisRequest): Promise<EmailAnalysis> {
    ...
    try {
      const response = await this.request("https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${this.apiKey}`, "content-type": "application/json" }, signal: controller.signal, body: JSON.stringify({
        ...
        input: JSON.stringify({ sender_name: input.senderName.slice(0, 200), subject: input.subject.slice(0, 300), current_message: input.preview.slice(0, 4000), conversation_history: (input.conversationMessages ?? []).slice(-12).map((message) => ({ direction: message.direction, body: message.body.slice(0, 2500) })), current_rule_category: input.currentClassification, relationship_context: input.relationshipContext?.slice(0, 300) ?? "unknown", verified_communication_profile: input.personaContext?.slice(0, 3000) ?? "not configured", owner_writing_examples: (input.styleExamples ?? []).slice(0, 6).map((value) => value.slice(0, 800)) }),
