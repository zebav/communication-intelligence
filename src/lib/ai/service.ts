...
export const emailAnalysisSchema = z.object({
  ...
  forwardingSuggestion: z.object({ recommended: z.boolean(), recipientRole: z.enum(["lawyer", "accountant", "advisor", "insurance_contact", "colleague", "other", "none"]), reason: z.string().max(400), introduction: z.string().max(1200) }),
  actionSuggestion: z.object({ detected: z.boolean(), type: z.enum(["contact_lookup", "web_research", "website_task", "form_completion", "none"]), task: z.string().max(600), reason: z.string().max(400), targetUrl: z.string().max(2000), requiresLogin: z.boolean(), contactIds: z.array(z.string()).max(3), confidence: z.number().min(0).max(1) }),
});
    ...
    try {
      const response = await this.request("https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${this.apiKey}`, "content-type": "application/json" }, signal: controller.signal, body: JSON.stringify({
        ...
        text: { format: { type: "json_schema", name: "email_intelligence", strict: true, schema: { type: "object", additionalProperties: false, properties: {
          ...
          forwardingSuggestion: { type: "object", additionalProperties: false, properties: { recommended: { type: "boolean" }, recipientRole: { type: "string", enum: ["lawyer", "accountant", "advisor", "insurance_contact", "colleague", "other", "none"] }, reason: { type: "string" }, introduction: { type: "string" } }, required: ["recommended", "recipientRole", "reason", "introduction"] },
          actionSuggestion: { type: "object", additionalProperties: false, properties: { detected: { type: "boolean" }, type: { type: "string", enum: ["contact_lookup", "web_research", "website_task", "form_completion", "none"] }, task: { type: "string" }, reason: { type: "string" }, targetUrl: { type: "string" }, requiresLogin: { type: "boolean" }, contactIds: { type: "array", maxItems: 3, items: { type: "string" } }, confidence: { type: "number", minimum: 0, maximum: 1 } }, required: ["detected", "type", "task", "reason", "targetUrl", "requiresLogin", "contactIds", "confidence"] },
