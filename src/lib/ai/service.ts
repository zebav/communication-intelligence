...
export const emailAnalysisSchema = z.object({
  ...
  actionSuggestion: z.object({ detected: z.boolean(), type: z.enum(["contact_lookup", "web_research", "website_task", "form_completion", "none"]), task: z.string().max(600), reason: z.string().max(400), targetUrl: z.string().max(2000), requiresLogin: z.boolean(), contactIds: z.array(z.string()).max(3), requiredFields: z.array(z.object({ key: z.string().min(1).max(80), label: z.string().min(1).max(120), kind: z.enum(["text","email","phone","date","username","password","account_number","other"]), description: z.string().max(240), sensitivity: z.enum(["personal","sensitive","restricted"]) })).max(12), confidence: z.number().min(0).max(1) }),
});
