...
async function authenticatedOwner() {
  ...
  return assurance?.currentLevel === "aal2" ? user : null;
}

export async function GET() {
  const user = await authenticatedOwner();
  if (!user) return NextResponse.json({ error: "Your session has expired or MFA is required." }, { status: 401 });
  const admin = createAdminClient();
  const { data, error } = await admin.from("profiles").select("preferences,updated_at").eq("id", user.id).maybeSingle();
...
export async function POST(request: Request) {
  ...
  const admin = createAdminClient();
  const { data: existing, error: readError } = await admin.from("profiles").select("preferences").eq("id", user.id).maybeSingle();
  if (readError || !existing) return NextResponse.json({ error: "Your profile record could not be found." }, { status: 500 });
  ...
  const { error: updateError } = await admin.from("profiles").update({ preferences: { ...preferences, universal_communication_profile: parsed.data }, updated_at: updatedAt }).eq("id", user.id);
  if (updateError) return NextResponse.json({ error: "Supabase rejected the profile update." }, { status: 500 });
  const { data: confirmed, error: confirmError } = await admin.from("profiles").select("preferences,updated_at").eq("id", user.id).maybeSingle();
