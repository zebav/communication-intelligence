...
function database({ connections = [{ id: "connection", owner_id: "owner", token_metadata: { phone_number_id: "100" } }], saveError = null as null | { code: string }, duplicate = false } = {}) {
  ...
  return { from: vi.fn((table: string) => {
    ...
    chain.maybeSingle = vi.fn(async () => {
      if (table !== "messages") return result;
      messageReads += 1;
      if (messageReads === 1) return { data: duplicate ? { id: "message" } : null, error: null };
      if (messageReads >= 3 && saveError?.code === "23505") return { data: { id: "message" }, error: null };
      return { data: saveError ? null : { id: "message" }, error: saveError };
    });
    return chain;
  }) };
}
describe("WhatsApp delivery acknowledgment", () => {
  ...
  it("acknowledges a concurrent duplicate instead of asking the provider to retry", async () => { mocks.admin.mockReturnValue(database({ saveError: { code: "23505" } })); const response = await POST(request()); expect(response.status).toBe(200); expect(await response.json()).toMatchObject({ imported: 0, duplicates: 1, failed: 0 }); expect(mocks.after).not.toHaveBeenCalled(); });
  it("authenticates but ignores Meta deliveries when YCloud is active", async () => { vi.stubEnv("WHATSAPP_ACTIVE_PROVIDER", "ycloud"); const response = await POST(request()); expect(response.status).toBe(200); expect(await response.json()).toMatchObject({ ignored: true, provider: "meta-direct", activeProvider: "ycloud" }); expect(mocks.admin).not.toHaveBeenCalled(); });
});
