...
export async function GET() {
  ...
  const hasDirectConnection = Boolean(
    process.env.WHATSAPP_ACCESS_TOKEN?.trim()
      && process.env.WHATSAPP_PHONE_NUMBER_ID?.trim()
      && process.env.WHATSAPP_BUSINESS_ACCOUNT_ID?.trim(),
  );
  if (hasDirectConnection) return NextResponse.json({ mode: "direct" });
  ...
  return NextResponse.json({ mode: "embedded", appId, configId, version: process.env.META_GRAPH_API_VERSION || "v26.0" });
}
