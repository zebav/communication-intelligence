...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return jsonError("Check the forwarding details and try again.", 400);
  ...
  if (!connection?.encrypted_credentials) return jsonError("Connect the originating Outlook account again before forwarding.", 409);
  ...
  try {
    ...
    const graphResponse = await fetch(`https://graph.microsoft.com/v1.0/me/messages/${encodeURIComponent(message.external_message_id)}/forward`, { method: "POST", headers: { authorization: `Bearer ${token.accessToken}`, "content-type": "application/json" }, body: JSON.stringify({ comment: parsed.data.comment, toRecipients: [{ emailAddress: { address: recipient } }] }), signal: AbortSignal.timeout(20_000) });
    ...
    const { error: auditError } = await supabase.from("audit_logs").insert({ owner_id: user.id, actor_id: user.id, action: "message.forwarded", object_type: "conversation", object_id: parsed.data.conversationId, source: "email", actor_type: "user", new_value: { provider: microsoftGraphConnector.id, recipient_person_id: parsed.data.recipientPersonId, recipient, forwarded_at: forwardedAt } });
    if (auditError) console.error("Microsoft forward succeeded but audit persistence failed", auditError.code);
    return NextResponse.json({ success: true, forwardedAt });
  } catch (error) {
    console.error("Microsoft forward failed", error instanceof Error ? error.message : "unknown");
    return jsonError("The email could not be forwarded. Try again.");
  }
}
