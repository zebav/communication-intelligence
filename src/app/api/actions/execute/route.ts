...
export async function POST(request: NextRequest) {
  ...
  const url = safeExternalActionUrl(typeof suggestion.targetUrl === "string" ? suggestion.targetUrl : ""); if (input.data.mode === "start" && suggestion.targetUrl && !url) return jsonError("The proposed website is not a permitted HTTPS address.", 422);
