...
export async function analyzeEmailWithAI(input: { messageId: string; conversationId: string }) {
  ...
  try {
    ...
    const { data: contactRows } = await supabase.from("people").select("id,display_name,organization,relationship_type,professional_specialty,jurisdiction").eq("owner_id", user.id).eq("entity_type", "person").order("last_contact_at", { ascending: false }).limit(300);
