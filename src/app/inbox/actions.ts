...
export async function analyzeEmailWithAI(input: { messageId: string; conversationId: string }) {
  ...
  try {
    ...
    const { count: personConversationCount } = conversation.person_id ? await supabase.from("conversations").select("id", { count: "exact", head: true }).eq("owner_id", user.id).eq("person_id", conversation.person_id) : { count: null };
